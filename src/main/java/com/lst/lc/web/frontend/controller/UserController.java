...
public class UserController {
	...
	public String notificationLetter(Model model, HttpSession session) {
		...
		if (letters.size() > 0) {
			for (int i = 0; i < letters.size(); i++) {
				...
				letterDao.updateState(letter.getId().getFromUid(), letter
						.getId().getToUid(), letter.getId().getTime(), 1);
			}
		}
	...
	public String read(Model model, HttpSession session, @PathVariable int uid,
			@PathVariable String time) throws Exception {
		User user = (User) session.getAttribute("loginUser");
